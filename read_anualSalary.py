...
input_df = input_df.drop(["Annual Salary"],axis=1)

# input_df = input_df.values.reshape(-1,1)
output_df = df["Annual Salary"].values.reshape(-1,1)
...
def scatter_graph():  
    ...
    x_column = 'Annual Salary'
    # Iterate over other columns for comparison
    for column in numeric_df.columns:
        if column != x_column:
            sns.scatterplot(x=x_column, y=column, data=numeric_df,)
